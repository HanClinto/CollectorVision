...
def _load_data() -> dict:
    ...
    return {
        "image_size": [1280, 960],
        "corners_normalized": [
            [0.310, 0.104], [0.662, 0.115], [0.672, 0.810], [0.295, 0.819]
        ],
        "sharpness": 0.0603,
...
def make_barcode(values: list[float], width: float = 5.0, height: float = 0.4,
                 n_show: int = 64) -> VGroup:
    ...
    for i, v in enumerate(normed):
        ...
        rect = Rectangle(width=cell_w, height=height, fill_color=color,
                         fill_opacity=1.0, stroke_width=0)
        rect.shift(RIGHT * (i * cell_w - width / 2 + cell_w / 2))
        rects.add(rect)
    border = Rectangle(width=width, height=height, stroke_color=WHITE,
...
def image_corner_to_manim(cx: float, cy: float, img_mob) -> np.ndarray:
    ...
    return np.array([center[0] + (cx - 0.5) * w,
                     center[1] + (0.5 - cy) * h,
                     0])


def step_badge(n: int, text: str) -> VGroup:
    circle = Circle(radius=0.28, color=ACCENT, fill_color=ACCENT,
...
def _img_or_rect(path, height, fill_color=BLUE_D):
    ...
    return Rectangle(width=height / aspect, height=height,
...
def play_title(s: Scene) -> None:
    title = Text("How does a computer\nrecognize a trading card?",
                 font_size=44, color=WHITE, line_spacing=1.3)
    subtitle = Text("CollectorVision — a three-step pipeline",
    ...
    ).arrange(DOWN, aligned_edge=LEFT, buff=0.3).next_to(subtitle, DOWN, buff=0.6)

    s.play(Write(title), run_time=1.5)
    s.play(FadeIn(subtitle, shift=UP * 0.2), run_time=0.8)
    s.play(LaggedStart(*[FadeIn(b, shift=RIGHT * 0.3) for b in steps],
...
def play_corner(s: Scene) -> None:
    ...
    caption = Text("A neural network looks at the full image...",
                   font_size=22, color=DIM_WHITE).to_edge(DOWN, buff=0.4)
    s.play(FadeIn(caption), run_time=0.5)
    s.wait(0.3)

    # Heatmap blobs at corner positions
    blobs = VGroup(*[
        Circle(radius=0.45, color=CORNER_COLORS[label],
               fill_color=CORNER_COLORS[label], fill_opacity=0.25, stroke_width=2
               ).move_to(image_corner_to_manim(cx, cy, img))
        for (cx, cy), label in zip(corners_norm, CORNER_LABELS)
    ])
    s.play(LaggedStart(*[GrowFromCenter(b) for b in blobs], lag_ratio=0.2),
           run_time=1.0)

    s.play(Transform(caption,
           Text("...and outputs probability distributions for each corner",
                font_size=22, color=DIM_WHITE).to_edge(DOWN, buff=0.4)),
           run_time=0.5)
    s.wait(0.3)
    ...
    # Dots snap to peak
    dots = VGroup(*[
        Dot(point=image_corner_to_manim(cx, cy, img), radius=0.09,
            color=CORNER_COLORS[label])
        for (cx, cy), label in zip(corners_norm, CORNER_LABELS)
    ])
    dot_labels = VGroup(*[
        Text(label, font_size=16, color=CORNER_COLORS[label]).next_to(
            Dot(point=image_corner_to_manim(cx, cy, img)), UP * 0.4 + RIGHT * 0.3,
            buff=0.05)
        for (cx, cy), label in zip(corners_norm, CORNER_LABELS)
    ])
    s.play(*[Transform(b, d) for b, d in zip(blobs, dots)], run_time=0.8)
    s.play(LaggedStart(*[FadeIn(l) for l in dot_labels], lag_ratio=0.15),
           run_time=0.6)

    polygon = Polygon(
        *[image_corner_to_manim(cx, cy, img) for cx, cy in corners_norm],
        color=HIGHLIGHT, stroke_width=2.5, fill_opacity=0,
    )
    s.play(Create(polygon), run_time=0.8)

    s.play(Transform(caption,
           Text("Corners located — card boundary defined",
                font_size=22, color=DIM_WHITE).to_edge(DOWN, buff=0.4)),
           run_time=0.4)
    ...
    gauge_bg = Rectangle(width=2.2, height=0.22, fill_color=DARK_GRAY,
                         fill_opacity=1, stroke_width=0)
    fill_frac = min(sharpness / 0.10, 1.0)
    gauge_fill = Rectangle(width=2.2 * fill_frac, height=0.22,
                           fill_color=HIGHLIGHT, fill_opacity=1, stroke_width=0)
...
def play_dewarp(s: Scene) -> None:
    ...
    polygon = Polygon(*quad_pts, stroke_color=HIGHLIGHT, stroke_width=2.5,
    ...
    card_label = Text("252 × 352 px\nalways the same size", font_size=18,
                      color=DIM_WHITE, line_spacing=1.2)
    card_label.next_to(dewarped, DOWN, buff=0.2)

    arrow = Arrow(LEFT * 0.4, RIGHT * 1.2, color=ACCENT, stroke_width=3,
    ...
    target = Rectangle(width=dewarped.width, height=dewarped.height,
                       stroke_color=HIGHLIGHT, stroke_width=2.5,
                       fill_color=BLUE_E, fill_opacity=0.15
                       ).move_to(dewarped.get_center())

    s.play(Transform(polygon, target), FadeOut(corner_dots), FadeOut(corner_lbls),
           run_time=1.2)
    s.play(FadeIn(dewarped), run_time=0.6)
    s.play(FadeIn(card_label), run_time=0.4)

    caption = Text("Card is now flat, front-facing, and ready for recognition",
                   font_size=22, color=DIM_WHITE).to_edge(DOWN, buff=0.4)
    s.play(FadeIn(caption), run_time=0.5)
    s.wait(1.5)
...
def play_embed(s: Scene) -> None:
    ...
    card_lbl = Text("Card image\n448 × 448 px", font_size=17, color=DIM_WHITE,
                    line_spacing=1.2).next_to(card_img, DOWN, buff=0.15)
    s.play(FadeIn(card_img), FadeIn(card_lbl), run_time=0.6)

    # Neural net funnel
    funnel = VGroup(*[
        Rectangle(width=w, height=0.28,
    ...
    ]).arrange(DOWN, buff=0.15).move_to(LEFT * 1.8 + UP * 0.3)
    nn_lbl = Text("MobileViT-XXS\n(Milo)", font_size=18, color=DIM_WHITE,
                  line_spacing=1.2).next_to(funnel, DOWN, buff=0.2)

    arrow_in = Arrow(card_img.get_right(), funnel.get_left(), color=ACCENT,
                     stroke_width=2.5, max_tip_length_to_length_ratio=0.12)
    s.play(GrowArrow(arrow_in), run_time=0.5)
    s.play(LaggedStart(*[FadeIn(l) for l in funnel], lag_ratio=0.1),
           FadeIn(nn_lbl), run_time=0.8)
    s.wait(0.3)

    output_lbl = Text("128 numbers", font_size=28, color=HIGHLIGHT
                      ).move_to(RIGHT * 2.0 + UP * 1.6)
    dim_note = Text("(L2-normalised float16 vector)", font_size=17, color=DIM_WHITE
                    ).next_to(output_lbl, DOWN, buff=0.1)
    arrow_out = Arrow(funnel.get_right(), output_lbl.get_left() + LEFT * 0.2,
    ...
    bc_lbl = Text("Embedding \"fingerprint\"", font_size=19, color=DIM_WHITE
                  ).next_to(barcode, DOWN, buff=0.18)
    arrow_bc = Arrow(output_lbl.get_bottom() + DOWN * 0.05,
                     barcode.get_top() + UP * 0.05, color=DIM_WHITE,
                     stroke_width=1.5, max_tip_length_to_length_ratio=0.12)
    s.play(GrowArrow(arrow_bc), run_time=0.4)
    s.play(LaggedStart(*[FadeIn(c) for c in barcode[0]], lag_ratio=0.01),
           FadeIn(barcode[1]), run_time=1.0)
    s.play(FadeIn(bc_lbl), run_time=0.4)

    insight = Text("Same card → similar fingerprint,\nregardless of lighting or angle",
...
def play_search(s: Scene) -> None:
    ...
    title.to_edge(UP, buff=0.3)
    caption = Text("Query fingerprint vs. 108,000 catalog embeddings",
                   font_size=20, color=DIM_WHITE).next_to(title, DOWN, buff=0.2)
    ...
    s.play(LaggedStart(*[FadeIn(c) for c in q_bc[0]], lag_ratio=0.005),
           FadeIn(q_bc[1]), FadeIn(q_lbl), run_time=0.7)

    # Arrow from query panel
    arrow = CurvedArrow(q_bc.get_right() + RIGHT * 0.05,
                        LEFT * 1.8 + UP * 1.6,
                        color=ACCENT, angle=-TAU / 8)
    ...
    for i, (score, cid) in enumerate(top_hits):
        ...
        if cid in hit_embs:
            ...
            cat_emb = (np.array(query_emb[:128]) * (0.9 if is_top else 0.3)
                       + rng.normal(0, 0.5, 128))
            cat_emb = (cat_emb / (np.linalg.norm(cat_emb) + 1e-9)).tolist()

        q_strip = make_barcode(query_emb, width=3.5, height=0.14, n_show=64
                               ).move_to(RIGHT * 0.3 + UP * (y + 0.17))
        c_strip = make_barcode(cat_emb, width=3.5, height=0.14, n_show=64
                               ).move_to(RIGHT * 0.3 + UP * (y - 0.01))

        score_color = HIGHLIGHT if is_top else WHITE
        score_text = Text(f"{score:.3f}", font_size=16 if is_top else 14,
                          color=score_color).move_to(RIGHT * 2.4 + UP * y)
        rank_text = Text(f"#{i+1}", font_size=14, color=DIM_WHITE
                         ).move_to(LEFT * 1.55 + UP * y)
        id_text = Text(cid[:8] + "…", font_size=11, color=DIM_WHITE
                       ).move_to(LEFT * 0.6 + UP * y)

        anims = [FadeIn(q_strip), FadeIn(c_strip), FadeIn(score_text),
                 FadeIn(rank_text), FadeIn(id_text)]

        if is_top:
            bg = Rectangle(width=9.0, height=row_height - 0.1,
                           fill_color=ManimColor("#002200"), fill_opacity=0.7,
                           stroke_color=HIGHLIGHT, stroke_width=1.5
    ...
    note = Text(f"Score: {top_score:.3f}  ← clear winner  "
                f"(gap to #2: {top_score - top_hits[1][0]:.3f})",
                font_size=20, color=HIGHLIGHT).to_edge(DOWN, buff=0.4)
...
def play_result(s: Scene) -> None:
    ...
    orig = _img_or_rect(ORIGINAL_IMAGE if ORIGINAL_IMAGE.exists() else SAMPLE_IMAGE, 3.4,
                        fill_color=GRAY)
    orig.move_to(LEFT * 4.2)

    arrow = Arrow(LEFT * 1.8, RIGHT * 0.2, color=ACCENT, stroke_width=3,
    ...
    result = VGroup(card_name, card_meta, score_grp).arrange(DOWN, buff=0.25
                                                              ).move_to(RIGHT * 2.5)
    ...
    timing = Text("Identified in < 1 second on CPU", font_size=20, color=DIM_WHITE
                  ).to_edge(DOWN, buff=0.6)
    footer = Text("collector_vision · pip install collector-vision",
                  font_size=18, color=ACCENT).next_to(timing, DOWN, buff=0.2)
...
class PipelineExplained(Scene):
    ...
    def construct(self):
        ...
        for fn in [play_title, play_corner, play_dewarp,
                   play_embed, play_search, play_result]:
            fn(self)
